...
int main (int argc, char* argv[]) {
  ...
  cv::setWindowProperty("Live stream", cv::WND_PROP_FULLSCREEN, cv::WINDOW_FULLSCREEN);
            ...
            if (by_top >= 0 && by_top < original_height) {
              *(original_data_tensor.data_ptr(bx, by_top, 0, 0)) = 1.0;
              if(box.c != UNKNOWN_CLASS) {
                ...
                *(original_data_tensor.data_ptr(bx, by_top, 1, 0)) = 0.0;
                *(original_data_tensor.data_ptr(bx, by_top, 2, 0)) = 0.0;
              }
            }
            if (by_bot >= 0 && by_bot < original_height) {
              *(original_data_tensor.data_ptr(bx, by_bot, 0, 0)) = 1.0;
              if(box.c != UNKNOWN_CLASS) {
                ...
                *(original_data_tensor.data_ptr(bx, by_bot, 1, 0)) = 0.0;
                *(original_data_tensor.data_ptr(bx, by_bot, 2, 0)) = 0.0;
              }
            }
          }
        }
            ...
            if (bx_top >= 0 && bx_top < original_width) {
              *(original_data_tensor.data_ptr(bx_top, by, 0, 0)) = 1.0;
              if(box.c != UNKNOWN_CLASS) {
                ...
                *(original_data_tensor.data_ptr(bx_top, by, 1, 0)) = 0.0;
                *(original_data_tensor.data_ptr(bx_top, by, 2, 0)) = 0.0;
              }
            }
            if (bx_bot >= 0 && bx_bot < original_width) {
              *(original_data_tensor.data_ptr(bx_bot, by, 0, 0)) = 1.0;
              if(box.c != UNKNOWN_CLASS) {
                ...
                *(original_data_tensor.data_ptr(bx_bot, by, 1, 0)) = 0.0;
                *(original_data_tensor.data_ptr(bx_bot, by, 2, 0)) = 0.0;
              }
            }
          }
        }
...
#pragma omp parallel for default(shared)
      ...
      for (unsigned int b = 0; b < output_boxes->size(); b++) {
        ...
        // Put box text
        if (box.c == UNKNOWN_CLASS)
          cv::putText(camera_frame, "Unknown", cv::Point(box.x - (box.w/2), box.y + 20 - (box.h/2)), cv::FONT_HERSHEY_PLAIN, 3.0, cv::Scalar(0,0,255));
        else
          cv::putText(camera_frame, class_manager.GetClassInfoById(box.c).first, cv::Point(box.x - (box.w/2), box.y + 20 - (box.h/2)), cv::FONT_HERSHEY_PLAIN, 3.0, cv::Scalar(255,255,255));
      }
